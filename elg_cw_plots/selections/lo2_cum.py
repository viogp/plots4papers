...
ax.set_xlabel('${\\rm log}_{10}(L\\rm{'+lline+'}/h^{-2}erg\, s^{-1})$')
...
for iz,zsnap in enumerate(snap_list):
    ...
    for ivol in range(nvol):
        ...
        if (os.path.isfile(gfile)):
            ...
            if (os.path.isfile(efile)):
                f = h5py.File(efile,'r')
                lum_ext = f['Output001/L_tot_'+line+'_ext'][:]
                lum = f['Output001/L_tot_'+line][:]
                ...
                for index in range(ntypes):
                    if index==0:
                        ...
                        g = f['Output001/mag_DES-g_o_tot_ext'][:] + tomag
                        r = f['Output001/mag_DES-r_o_tot_ext'][:] + tomag
                        z = f['Output001/mag_DES-z_o_tot_ext'][:] + tomag
                        rz = r-z ; gr = g-r
                        ...
                        ib = bands[index-1]
                        mag = f['Output001/mag_'+ib+'_o_tot_ext'][:] + tomag
                        icut = mcuts[index-1]
                        fluxcut = fcuts[index-1]
                        lcut = emission_line_luminosity(fluxcut,zz)
    ...
    # Write output
    outfile = '/cosma5/data/durham/violeta/lines/cosmicweb/selections/'+\
          model+'lo2_cum_sn'+str(zsnap)+'.dat'
    with open(outfile,'w') as outf:
        outf.write('# log10(L[OII]/h-2erg s-1) log10(ngal for ')
