        ...
        if (os.path.isfile(gfile)):
            ...
            if (os.path.isfile(efile)):
                ...
                if(len(lum_ext) ==len(sfr)):
                    ...
                    ind  = np.where((lum_ext>lcut) & (cen<1))
                    if (np.shape(ind)[1] > 0.):
                        ll = np.log10(lum_ext[ind]) + 40.
                    ...
                    # Satellites
                    ind  = np.where((lum_ext>lcut) & (cen>0))
                    if (np.shape(ind)[1] > 0.):
                        ll = np.log10(lum_ext[ind]) + 40.
                        #ll = np.log10(lum[ind]) + 40.
                        H, bins_edges = np.histogram(ll,bins=np.append(lbins,lmax))
                    ...
                    index = 2
                    # Quiescent
                    ind  = np.where((lum_ext>lcut) & (tburst>tsf))
                    if (np.shape(ind)[1] > 0.):
                        ll = np.log10(lum_ext[ind]) + 40.
                        #ll = np.log10(lum[ind]) + 40.
                        H, bins_edges = np.histogram(ll,bins=np.append(lbins,lmax))
                    ...
                    index = 4
                    # Big.
                    ind  = np.where((lum_ext>lcut) & (r50>0.5))
                    if (np.shape(ind)[1] > 0.):
                        ll = np.log10(lum_ext[ind]) + 40.
                        #ll = np.log10(lum[ind]) + 40.
                        H, bins_edges = np.histogram(ll,bins=np.append(lbins,lmax))
                    ...
                    index = 5
                    # Spheroid.
                    ind  = np.where((lum_ext>lcut) & (BoT>0.5))
                    if (np.shape(ind)[1] > 0.):
                        ll = np.log10(lum_ext[ind]) + 40.
                        #ll = np.log10(lum[ind]) + 40.
                        H, bins_edges = np.histogram(ll,bins=np.append(lbins,lmax))
