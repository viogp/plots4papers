...
envsumfile = path+'env_files/'+model+'fenv_elg.txt'
sumfile = open(envsumfile, 'w')
sumfile.write('# Fraction ELGs, LSE, Survey, Snapshot, Vweb/Pweb \n')
sumfile.close()
    ...
    for iiz, sn in enumerate(snaps):
        ...
        for survey in surveys:
            ...
            for iie, ienv in enumerate(np.unique(env)):
                ...
                fenv = float(np.shape(ind)[1])/ntot
                sumfile.write('{:.2f} {}\n'.format(fenv,elabels[iie])) 
